...
public class ItemTime {
    ...
    public static final byte TEXT_NONG = 4;
    ...
    public static final int TIME_NONG = 300000;
    public static final byte TX = 33;
    private Player player;

    public boolean isNong;
    public boolean isUseMiaKhongLo;
    public boolean isUseMiaThom;
    public boolean isUseMiaSauRieng;
    ...
    public long lastTimeNong;
    public long lastTimeMiaKhongLo;
    public long lastTimeMiaThom;
    public long lastTimeMiaSauRieng;
...
//                Service.getInstance().Send_Info_NV(this.player);
        ...
        if (isNong) {
            if (Util.canDoWithTime(lastTimeNong, TIME_NONG)) {
                isNong = false;
                Service.gI().point(player);
            }
        }
        if (isUseMiaKhongLo) {
            if (Util.canDoWithTime(lastTimeMiaKhongLo, TIME_ITEM)) {
                isUseMiaKhongLo = false;
                Service.gI().point(player);
            }
        }
        if (isUseMiaThom) {
            if (Util.canDoWithTime(lastTimeMiaThom, TIME_ITEM)) {
                isUseMiaThom = false;
                Service.gI().point(player);
            }
        }
        if (isUseMiaSauRieng) {
            if (Util.canDoWithTime(lastTimeMiaSauRieng, TIME_ITEM)) {
                isUseMiaSauRieng = false;
                Service.gI().point(player);
            }
        }
    }
