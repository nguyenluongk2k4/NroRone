...
public class PetService {
    ...
    public void createNormalPet(Player player, int gender, byte... limitPower) {
        new Thread(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.getInstance().chatJustForMe(player, player.pet, "Xin hãy thu nhận con làm đệ tử");
            } catch (Exception e) {
            }
        }).start();
    }
    ...
    public void createMabuPet(Player player, int gender, byte... limitPower) {
        new Thread(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.getInstance().chatJustForMe(player, player.pet, "Xin hãy thu nhận con làm đệ tửOa oa oa...");
            } catch (Exception e) {
            }
        }).start();
    }
    ...
    private void createNewPet(Player player, boolean isMabu, boolean isBerus,boolean isPic,boolean isCumber, boolean Gokunecon,boolean Cadicnecon,byte... gender) {
        ...
        pet.name = "$" + (isMabu ? "MaBư" : isBerus ? "Gohan Buu" : isPic ? "Evil Buu" : isCumber ? "Cumber SSJ2" : Gokunecon ? "Vegeta Blue" : Cadicnecon ? "Songoku Ultra"  : "Đệ tử");
