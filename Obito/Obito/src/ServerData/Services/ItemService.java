...
//            }
    ...
    public Item createNewItem(short tempId, int quantity) {
        ...
        public Item createNewDanhHieu(short tempId, int sd, int hp, int ki) {
        Item item = new Item();
        item.template = getTemplate(tempId);
        item.quantity = 1;
        item.createTime = System.currentTimeMillis();
        item.content = item.getContent();
        item.info = item.getInfo();
        item.itemOptions.add(new ItemOption(50,sd));
        item.itemOptions.add(new ItemOption(77, hp));
        item.itemOptions.add(new ItemOption(103, ki));
